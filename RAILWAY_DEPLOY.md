...
**Приоритет 1: DATABASE_URL (рекомендуется)**
- `DATABASE_URL` - полный URL базы данных в формате: `postgresql://user:password@host:port/database`
- Если Railway предоставляет `DATABASE_URL`, он будет использован автоматически
- **Важно**: Spring Boot автоматически парсит `DATABASE_URL` и извлекает username/password из URL

**Приоритет 2: Стандартные Railway переменные**
...
**Приоритет 3: Альтернативные переменные**
...
- Приложение автоматически использует переменные окружения в порядке приоритета
- Если переменные не установлены, используются значения по умолчанию для локальной разработки
- `DATABASE_URL` имеет наивысший приоритет и будет использован, если он установлен
